...
@pytest.mark.asyncio
async def test_ai_task_uses_one_tools_disabled_turn_after_default_tool_rounds(
    ai_task_module,
    monkeypatch,
):
    calls = []
    ...
    assert [call["allow_tools"] for call in calls] == [True] * 8 + [False]
    assert [bool(call["tools"]) for call in calls] == [True] * 8 + [False]
    assert all(call["tools"][0]["name"] == "HassTurnOn" for call in calls[:8])
