...
async def test_codex_input_from_chat_log_preserves_history_tools_and_results(
    ...
    result = await conversation_module.transcript.codex_input_from_chat_log(object(), chat_log)
...
async def test_codex_stream_to_assistant_deltas_yields_text_and_tool_inputs(
    ...
    deltas = [
        delta
        async for delta in conversation_module.turn_runtime.codex_stream_to_assistant_deltas(
            stream(),
...
def test_codex_tools_from_chat_log_converts_ha_llm_api_tools(conversation_module):
    ...
    result = conversation_module.transcript.codex_tools_from_chat_log(FakeChatLog(llm_api=llm_api))
    ...
    assert conversation_module.transcript.codex_tools_from_chat_log(
...
def test_codex_tools_adds_opt_in_web_search_without_ha_tools(conversation_module):
    assert (
        conversation_module.transcript.codex_tools_from_chat_log(
            FakeChatLog(), enable_web_search=False
        )
        == []
    )
    assert conversation_module.transcript.codex_tools_from_chat_log(
...
async def test_stream_codex_turn_into_chat_log_calls_chat_log_stream_api(
    ...
    tool_requested = await conversation_module.turn_runtime.stream_codex_turn_into_chat_log(
        chat_log=chat_log,
        codex=codex,
...
async def test_codex_input_from_chat_log_translates_image_attachments(
    ...
    result = await conversation_module.transcript.codex_input_from_chat_log(hass, chat_log)

    content = result[0]["content"]
    assert content[0] == {"type": "input_text", "text": "describe this"}
    assert content[1]["type"] == "input_image"
    assert content[1]["image_url"].startswith("data:image/png;base64,")
    assert hass.executor_jobs[0][0] is conversation_module.transcript.image_attachments_for_codex
...
def test_image_attachments_rejects_too_many_images(
    ...
    monkeypatch.setattr(conversation_module.transcript, "MAX_IMAGE_ATTACHMENTS", 2)
    attachments = []
    ...
    with pytest.raises(ValueError, match="at most 2 image attachments"):
        conversation_module.transcript.image_attachments_for_codex(attachments)
...
def test_image_attachments_rejects_aggregate_byte_limit(
    ...
    monkeypatch.setattr(conversation_module.transcript, "MAX_TOTAL_IMAGE_ATTACHMENT_BYTES", 5)
    attachments = []
...
def test_image_attachment_growth_is_read_with_a_hard_bound(
    ...
    monkeypatch.setattr(conversation_module.transcript, "MAX_IMAGE_ATTACHMENT_BYTES", 5)
    monkeypatch.setattr(conversation_module.transcript, "MAX_TOTAL_IMAGE_ATTACHMENT_BYTES", 5)
    ...
    with pytest.raises(ValueError, match="per-file size limit"):
        conversation_module.transcript.image_attachments_for_codex([attachment])
...
def test_trim_codex_input_items_drops_orphaned_tool_outputs(conversation_module):
    ...
    result = conversation_module.transcript.retain_complete_turns(input_items, max_items=4)
...
def test_trim_codex_input_items_leaves_short_history_unchanged(conversation_module):
    ...
    assert (
        conversation_module.transcript.retain_complete_turns(input_items, max_items=24)
        is input_items
    )
...
def test_trim_codex_input_items_keeps_complete_turn_at_retained_boundary(
    ...
    assert conversation_module.transcript.retain_complete_turns(input_items, max_items=4) == [
        {"role": "user", "content": "retained"},
        {
            "type": "function_call",
            "name": "BoundaryTool",
...
def test_trim_codex_input_items_rejects_oversize_current_turn(conversation_module):
    ...
    with pytest.raises(ValueError, match="contains 4 items; maximum is 3"):
        conversation_module.transcript.retain_complete_turns(input_items, max_items=3)
...
async def test_five_tool_rounds_force_one_tools_disabled_final_synthesis(conversation_module):
    ...
    await conversation_module.turn_runtime.run_tool_rounds(
        max_tool_rounds=5,
...
async def test_native_transcript_survives_tool_round_and_final_synthesis(
    ...
    async def run_iteration(_round_number, allow_tools):
        input_items = await conversation_module.transcript.codex_input_from_chat_log(
            object(), chat_log
        )
    ...
    next_turn_input = await conversation_module.transcript.codex_input_from_chat_log(
        object(), chat_log
    )
