...
The options flow keeps **Everyday settings** open and leaves **Advanced chat settings**, **Image-generation defaults**, and **Runtime / Orchestration** collapsed until needed. Everyday controls include the chat model, response length, and hosted web search. Advanced controls hold the system prompt and reasoning effort. Runtime controls provide bounded tool-round and Codex transport timeouts; a stream read timeout of `0` means disabled/unlimited for quiet Responses SSE streams.
